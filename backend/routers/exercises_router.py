...
@router.patch("/{exercise_id}/image", response_model=schemas.ExerciseOut)
def update_exercise_image(
    exercise_id: int,
    payload: schemas.ExerciseImageUpdate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    exercise = (
    ...
    if not exercise:
        raise HTTPException(status_code=404, detail="Exercise not found")
    exercise.custom_image = payload.custom_image
    db.commit()
    db.refresh(exercise)
    return exercise


@router.patch("/{exercise_id}/rest", response_model=schemas.ExerciseOut)
def update_exercise_rest(
    exercise_id: int,
    payload: schemas.ExerciseRestUpdate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    exercise = (
    ...
    if not exercise:
        raise HTTPException(status_code=404, detail="Exercise not found")
    exercise.rest_seconds = payload.rest_seconds
    db.commit()
    db.refresh(exercise)
    return exercise
