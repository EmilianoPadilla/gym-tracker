  ...
  return (
    <div className="fixed bottom-5 right-5 z-40">
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-full border shadow-lg px-4 py-3 font-display font-semibold ${
          running ? "bg-panelraised border-brasslight text-chalk" : "bg-panel border-hairline text-chalkdim"
        }`}
      >
        <ClockIcon />
        {secondsLeft !== null ? formatClock(secondsLeft) : formatClock(minutes * 60 + seconds)}
      </button>
    </div>
  );
}
